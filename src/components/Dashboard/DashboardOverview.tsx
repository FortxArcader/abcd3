...
const DashboardOverview: React.FC = () => {
    ...
    const outwardCount = documents.filter(
        (doc) => doc.type === "outward"
    ).length;
    ...
    const recentActivity = documents.slice(0, 4).map((doc) => ({
        id: doc.id,
        action:
            doc.type === "inward"
                ? "New Inward DAK Registered"
                : "New Outward DAK Created",
    ...
    const departmentStats = documents
        .reduce((acc: any[], doc) => {
            ...
            if (existing) {
                ...
                if (["received", "under_process"].includes(doc.status))
                    existing.pending++;
            } else {
                acc.push({
                    name: deptName,
                    ...
                    pending: ["received", "under_process"].includes(doc.status)
                        ? 1
                        : 0,
                });
            }
                        ...
                        <div className="space-y-4">
                            {recentActivity.length > 0 ? (
                                ...
                                <div className="text-center py-8 text-gray-500">
                                    ...
                                    <p className="text-sm">
                                        Create your first DAK document to get
                                        started
                                    </p>
                                </div>
                            )}
                        ...
                        <div className="space-y-4">
                            {departmentStats.length > 0 ? (
                                ...
                                <div className="text-center py-8 text-gray-500">
                                    ...
                                    <p className="text-sm">
                                        Data will appear once DAK documents are
                                        created
                                    </p>
                                </div>
                            )}
